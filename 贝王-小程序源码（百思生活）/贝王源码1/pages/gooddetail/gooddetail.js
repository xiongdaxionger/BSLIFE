...
Page({
    ...
    popViewAnimation: function(show) {
        ...
        this.setData({
            ...
            translateAnimation: animation.translateY(show ? (-220) : (height - 165)).step().export()
        })
    },
    //弹出规格动画
    ...
    canvas_inits: function (img){
      ...
      var imgPath = img;
      var bgImgPath = that.data.xcxcodeimg;
      ctx.setFillStyle('white')
      ctx.fillRect(0, 0, 320, 420);
      ctx.drawImage(imgPath, 0, 10, 320, 320);
      ctx.drawImage(bgImgPath, 220, 360, 60, 60);
      ctx.setFontSize(16)
      ctx.setFillStyle('#111111')
      ctx.fillText(that.data.model.name.slice(0, 20), 20, 330)
      ctx.fillText(that.data.model.name.slice(20), 20, 350)
      ctx.setFontSize(20)
      ctx.setFillStyle('#f73030')
      ctx.fillText(that.data.model.showPrice, 20, 380)
      ctx.setFontSize(14)
      ctx.setFillStyle('#cccccc')
      ctx.setFontSize(12)
      ctx.fillText('长按识别小程序码访问', 20, 400)
      ctx.draw(true,function (e) {
        that.transform_img()
      })
    },
    transform_img: function () {/*将画布转化成图片*/
      var that = this;
          ...
          wx.showModal({
            ...
            success: function (res) {
              if (res.confirm) {
                ...
                wx.redirectTo({
                  url: '/pages/gooddetail/gooddetail?productID=' + that.data.productID
                })
              }
            }
          })
    ...
    img_loaded:function(){
      if(this.data.doing_loading){return}
      this.setData({
        doing_loading:true
      })
      var that = this;
      wx.downloadFile({
        url: that.data.xcxcodeimg,
        success: function (e) {
          that.setData({
            xcxcodeimg: e.tempFilePath
          })
        }
      })
      wx.downloadFile({
        url: that.data.model.images[0],
        success: function (res) {
          // 只要服务器有响应数据，就会把响应内容写入文件并进入 success 回调，业务需要自行判断是否下载到了想要的内容
          if (res.statusCode === 200) {
            that.canvas_inits(res.tempFilePath);
          }
        }
      })
    },
    ...
    // 获取小程序码
    get_xcxcode:function(){
      var that = this
      app.request({
        "method": "pos.cartweb.get_access_token"
      }, function (data) {
        var productID = that.data.productID
        app.request({
          "method": "pos.cartweb.get_xcxcode",
          "path": 'pages/gooddetail/gooddetail?productID=' + productID,
          "width":'430',
          "access_token": data.data
        }, function (res) {
          that.data.xcxcodeimg = res.data;
