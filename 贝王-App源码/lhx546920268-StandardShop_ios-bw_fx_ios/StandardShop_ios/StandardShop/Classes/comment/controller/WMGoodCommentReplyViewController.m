//
//  WMGoodCommentReplyViewController.m
//  StandardShop
//
...
@interface WMGoodCommentReplyViewController ()<SeaHttpRequestDelegate,UITextViewDelegate,UIGestureRecognizerDelegate>
...
@implementation WMGoodCommentReplyViewController
...
- (void)viewDidLoad {
    ...
    CGFloat height = ![NSString isEmpty:self.codeURL] ? controlHeight * 2 + _separatorLineWidth_ : controlHeight;
...
- (void)send:(UIButton*) btn
{
    ...
    {
        [[AppDelegate instance] alertMsg:@"请输入回复内容"];
        return;
    }

    if(![NSString isEmpty:self.codeURL] && [NSString isEmpty:self.imageCodeView.textField.text])
    {
    ...
    self.requesting = YES;
    [self.httpRequest downloadWithURL:SeaNetworkRequestURL dic:[WMCommentOperation goodCommentReplyParamWithInfo:self.info content:self.textView.text code:self.imageCodeView.textField.text]];
}
...
- (void)httpRequest:(SeaHttpRequest *)request didFinishedLoading:(NSData *)data
{
    self.requesting = NO;
    self.showNetworkActivity = NO;
    id info = [WMCommentOperation goodCommentReplyResultFromData:data];
    if(info)
    {
        if([info isKindOfClass:[NSString class]])
        {
            [[AppDelegate instance] alertMsg:info];
            !self.replyCompletionHandler ?: self.replyCompletionHandler(nil);
        }
        else
        {
            self.info.partialReplyHeight = 0;
            self.info.replyHeight = 0;
            self.info.replyInfos = info;
            !self.replyCompletionHandler ?: self.replyCompletionHandler(self.info);
        }
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if(textView.height != size.height + margin * 2)
    {
        ...
        self.contentView.height = ![NSString isEmpty:self.codeURL] ? self.imageCodeView.bottom : textView.bottom;
