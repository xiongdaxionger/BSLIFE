 ...
 *@param identifier 请求标识 来识别是哪个请求
 */
typedef void(^SeaNetworkQueueRequestCompletionHandler)(SeaNetworkQueue *queue, NSData *data, NSInteger code, NSString *identifier);
 ...
 *@param identifier 请求标识 来识别是哪个请求
 */
typedef void(^SeaNetworkQueueRequestProgressHandler)(SeaNetworkQueue *queue, float uploadProgress, float downloadProgress, NSString *identifier);
...
/**更新某个请求的进度条
 */
- (void)networkQueue:(SeaNetworkQueue *)queue didUpdateUploadProgress:(float) progress downloadProgress:(float) downlodProgress identifier:(NSString*) identifier;
