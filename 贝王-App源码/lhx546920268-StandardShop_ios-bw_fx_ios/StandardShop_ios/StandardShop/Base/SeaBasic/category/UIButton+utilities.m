...
- (void)setButtonIconToTopAndTitleBottomWithInterval:(CGFloat) interval
{
    ...
    if (@available(iOS 11.0, *)) {
        if(alignment == UIControlContentHorizontalAlignmentLeading){
            alignment = UIControlContentHorizontalAlignmentLeft;
        }else if(alignment == UIControlContentHorizontalAlignmentTrailing){
    ...
    switch (alignment)
    {
        ...
        {
            [self setTitleEdgeInsets:UIEdgeInsetsMake(contentHeight / 2.0, - ((self.width - self.titleLabel.width + image.size.width) / 2.0 - (self.width - contentWidth) / 2.0), 0, 0)];
            [self setImageEdgeInsets:UIEdgeInsetsMake(0, (self.width - image.size.width) / 2.0 - (self.width - contentWidth) / 2.0, contentHeight / 2.0, -((self.width - image.size.width) / 2.0 - (self.width - contentWidth) / 2.0))];
        }
            break;
