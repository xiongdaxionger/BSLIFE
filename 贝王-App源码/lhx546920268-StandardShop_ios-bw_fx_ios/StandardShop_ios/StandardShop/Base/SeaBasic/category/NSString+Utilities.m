...
- (NSString*)formatTelPhoneNumber
{
    ...
    if(number.length >= 4)
    {
        if(number.length > WMTelPhoneNumberInputLimitMax)
        {
            number = [number stringByReplacingCharactersInRange:NSMakeRange(WMTelPhoneNumberInputLimitMax, number.length - WMTelPhoneNumberInputLimitMax) withString:@""];
        }
