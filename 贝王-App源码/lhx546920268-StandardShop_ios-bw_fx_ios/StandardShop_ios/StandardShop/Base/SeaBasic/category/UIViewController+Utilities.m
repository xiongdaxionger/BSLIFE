...
- (CGFloat)contentHeight
{
    ...
    if(existNav)
    {
        ...
        NSLog(@"navigationbarheight%.2f",self.navigationBarHeight);
    ...
    if(self.tabBarController && !self.tabBarController.tabBar.hidden && !self.hidesBottomBarWhenPushed)
    {
        ...
        NSLog(@"tabbarheight%.2f",self.tabBarController.tabBar.height);
    ...
    if(self.Sea_TabBarController && !self.Sea_TabBarController.tabBar.hidden && !self.hideTabBar)
    {
        ...
        NSLog(@"seatabbarheight%.2f",self.Sea_TabBarController.tabBar.height);
    ...
    if(!self.navigationController.toolbar.hidden && !self.hidesBottomBarWhenPushed && !self.navigationController.toolbar.translucent)
    {
        ...
        NSLog(@"toolbarheight%.2f",self.toolBarHeight);
    ...
    if(![UIApplication sharedApplication].statusBarHidden && !self.navigationController.navigationBar.translucent)
    {
        ...
        NSLog(@"statusheight%.2f",self.toolBarHeight);
...
- (void)setBackItem:(BOOL)backItem
{
    if(backItem)
    {
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, image.size.width + 10.0, 35)];
...
- (void)alertMsg:(NSString*) msg
{
    if(!self.promptView)
    {
        CGFloat y = (self.contentHeight - _seaAlertViewHeight_) / 2.0;
        if(y + _seaAlertViewHeight_ + 256.0 > self.contentHeight)
        {
            y = self.contentHeight - 256.0 - _seaAlertViewHeight_;
        }
        
        SeaPromptView *alertView = [[SeaPromptView alloc] initWithFrame:CGRectMake((self.view.width - _seaAlertViewWidth_) / 2.0, y, _seaAlertViewWidth_, _seaAlertViewHeight_) message:msg];
