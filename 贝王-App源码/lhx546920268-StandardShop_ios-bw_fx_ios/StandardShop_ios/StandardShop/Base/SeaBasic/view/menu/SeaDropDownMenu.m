...
- (void)setCellHighlight:(BOOL) highlight forIndex:(NSInteger) index
{
    ...
    cell.imageView.highlighted = highlight && _shouldHighlighted;
    cell.titleLabel.textColor = (highlight && _shouldHighlighted) ? _buttonHighlightTitleColor : _buttonNormalTitleColor;
}
...
- (void)handleCancelTap:(UITapGestureRecognizer*) tap
{
    ...
    if(!_keepHighlightWhenDismissList)
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SeaDropDownMenuCell *cell = [self cellForIndex:_selectedIndex];
    cell.titleLabel.text = [item.titleLists objectAtIndex:indexPath.row];
    [cell setNeedsLayout];
