...
{
    int page = self.visibleIndex; // 获取当前的page
...
{
    if (scrollView.contentOffset.x > (self.numberOfCells - 1) * _width_ && [self.delegate respondsToSelector:@selector(scrollViewDraggingLastPage:)]) {
        
        [self.delegate scrollViewDraggingLastPage:self];
    }
