...
@interface UUChart ()<UULineChartDelegate>
...
#pragma mark- UULineChart delegate

- (void)lineChart:(UULineChart *)chart didTouchAtIndex:(NSInteger)index
{
    if([self.dataSource respondsToSelector:@selector(UUChart:didTouchAtIndex:inPoint:)])
    {
        [self.dataSource UUChart:self didTouchAtIndex:index inPoint:_lineChart.indicatorLine.frame.origin];
    }
}

- (void)lineChart:(UULineChart *)chart didEndTouchAtIndex:(NSInteger)index
{
    if([self.dataSource respondsToSelector:@selector(UUChart:didEndTouchAtIndex:)])
    {
        [self.dataSource UUChart:self didEndTouchAtIndex:index];
    }
}
