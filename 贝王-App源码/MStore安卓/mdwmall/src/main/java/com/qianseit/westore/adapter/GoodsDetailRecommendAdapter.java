...
public class GoodsDetailRecommendAdapter extends BaseAdapter implements View.OnClickListener,ShareViewDataSource{
	...
	private JSONObject mSharedJSON;
	
	public GoodsDetailRecommendAdapter(DoFragment context ,VolleyImageLoader imageLoader,View rootView) {
		mContext = context;
		mImageLoader = imageLoader;
		mRootView=rootView;
		mShareViewPopupWindow = new ShareViewPopupWindow(mContext.mActivity);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.textview_levels.setText(all.optString("member_lv_name"));
...
//			holder.button_relateds.setText("+关注");
			holder.button_relateds.setText("");
...
//			float width = Float.valueOf(wm.getDefaultDisplay().getWidth());
			p.width = width - Util.dip2px(mContext.mActivity, 10);
			p.height = width - Util.dip2px(mContext.mActivity, 10);
			...
			if (v.getId() == R.id.img_brand_logos || v.getId() == R.id.textview_names) {
				...
				mContext.startActivity(AgentActivity
						.intentForFragment(mContext.mActivity,
								AgentActivity.FRAGMENT_RECOMMEND_HOME)
						.putExtra(Run.EXTRA_VALUE,
				...
				mContext.startActivity(AgentActivity.intentForFragment(mContext.mActivity,
						 AgentActivity.FRAGMENT_RECOMMEND_GOODS_COMMENT).putExtra("id",
								 all.optString("id")));
				...
				mSharedJSON=all; 
				mShareViewPopupWindow.showAtLocation(mRootView, android.view.Gravity.BOTTOM, 0, 0);
				//分享
...
//				morePopWindow.showPopupWindow(v);
				...
				mContext.startActivity(AgentActivity.intentForFragment(mContext.mActivity,
						 AgentActivity.FRAGMENT_ACCO_POINTS).putExtra(Run.EXTRA_DETAIL_TYPE,
								 AccoPointsFragment.POINTS_RULE));
			}
		}
	}
	...
	private class GetRecommend implements JsonTaskHandler{
		...
		public JsonRequestBean task_request() {
			JsonRequestBean requestBean = new JsonRequestBean(Run.API_URL, "mobileapi.goods.getcomment");
	...
	class AddPraiseTask implements JsonTaskHandler {
		...
		public AddPraiseTask(JSONObject all){
			this.meber_Id = AgentApplication.getLoginedUser(mContext.mActivity).getMember().getMember_id();
...
//			bean.addParams("member_id", meber_Id);
	...
	private class CancelAttentionTaskTask implements JsonTaskHandler {
		...
		public CancelAttentionTaskTask(JSONObject temp){
			...
			this.fansId = AgentApplication.getLoginedUser(mContext.mActivity).getMember().getMember_id();
		}
	...
	@Override
	public String getShareText() {
		return mSharedJSON.optString("goods_name");
	}

	@Override
	public String getShareImageFile() {
		return CacheUtils.getImageCacheFile(mSharedJSON.optString("image"));
	}
	...
	@Override
	public String getShareUrl() {
		return String.format(Run.RECOMMEND_URL,mSharedJSON.optString("id"));
	}
