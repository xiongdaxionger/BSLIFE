...
//	private static final String TAG = "ViewPagerTabs";
	...
	private final int SHADOW_WIDTH = 35;
	...
	private int mBackgroundColorPressed = 0x2243797F;

	private int mTextColor = 0xFFececec;
	private int mTextColorCenter = 0xFFFFFFFF;
	...
	public ViewPagerTabs(Context context, AttributeSet attrs, int defStyle) {
		...
		final TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ViewPagerTabs, defStyle, 0);

		mBackgroundColorPressed = a.getColor(
				R.styleable.ViewPagerTabs_backgroundColorPressed,
				mBackgroundColorPressed);

		mTextColor = a.getColor(R.styleable.ViewPagerTabs_textColors,
				mTextColor);
		mTextColorCenter = a.getColor(
				R.styleable.ViewPagerTabs_textColorCenter, mTextColorCenter);

		mLineColorCenter = a.getColor(
				R.styleable.ViewPagerTabs_lineColorCenter, mLineColorCenter);
		mLineHeight = a.getDimensionPixelSize(
				R.styleable.ViewPagerTabs_lineHeight, mLineHeight);

		mTextSize = a.getDimension(R.styleable.ViewPagerTabs_textSizes,
				mTextSize);

		mTabPaddingLeft = a.getDimensionPixelSize(
				R.styleable.ViewPagerTabs_tabPaddingLeft, mTabPaddingLeft);
		mTabPaddingTop = a.getDimensionPixelSize(
				R.styleable.ViewPagerTabs_tabPaddingTop, mTabPaddingTop);
		mTabPaddingRight = a.getDimensionPixelSize(
				R.styleable.ViewPagerTabs_tabPaddingRight, mTabPaddingRight);
		mTabPaddingBottom = a.getDimensionPixelSize(
				R.styleable.ViewPagerTabs_tabPaddingBottom, mTabPaddingBottom);

		mOutsideOffset = a.getDimensionPixelSize(
				R.styleable.ViewPagerTabs_outsideOffset, mOutsideOffset);
	...
	public void setViewPager(ViewPager pager) {
		...
		if (!(pager.getAdapter() instanceof ViewPagerTabProvider)) {
			throw new IllegalStateException(
					"The pager's adapter has to implement ViewPagerTabProvider.");
		}
	...
	/**
	 * Binds the {@link ViewPager} to this instance and sets the current
	 * position
	 * 
	 * @param pager
	 *            An instance of {@link ViewPager}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (c != null) {
			...
			setMeasuredDimension(
					...
					resolveSize(tab.getMeasuredHeight() + this.getPaddingTop()
							+ this.getPaddingBottom(), heightMeasureSpec));

		} else {
			setMeasuredDimension(
					resolveSize(0, widthMeasureSpec),
					resolveSize(this.getPaddingTop() + this.getPaddingBottom(),
							heightMeasureSpec));
		}
		...
		if (isFirstMeasurement) {
			...
			if (mOutsideOffset < 0)
				mOutsideOffset = getMeasuredWidth();
	...
	private void measureChildren() {
		...
		for (int i = 0; i < count; i++) {
			LayoutParams layoutParams = getChildAt(i)
					.getLayoutParams();
			final int widthSpec = MeasureSpec.makeMeasureSpec(
					layoutParams.width, MeasureSpec.EXACTLY);
			final int heightSpec = MeasureSpec.makeMeasureSpec(
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (diff <= highlightOffset) {
				...
				final int y = 100 * diff / x1;
				tab.setCenterPercent(100 - y);
			} else {
				tab.setCenterPercent(0);
			}

			tab.layout(tab.layoutPos, this.getPaddingTop(),
					tab.layoutPos + tab.getMeasuredWidth(),
					this.getPaddingTop() + tab.getMeasuredHeight());
	...
	private void calculateNewPositions(boolean firstTime) {
		...
		{
			ViewPagerTab leftOutside = pos > 1 ? (ViewPagerTab) getChildAt(pos - 2)
					: null;
			ViewPagerTab left = pos > 0 ? (ViewPagerTab) getChildAt(pos - 1)
					: null;
			ViewPagerTab center = (ViewPagerTab) getChildAt(pos);
			ViewPagerTab right = pos < getChildCount() - 1 ? (ViewPagerTab) getChildAt(pos + 1)
					: null;
			ViewPagerTab rightOutside = pos < getChildCount() - 2 ? (ViewPagerTab) getChildAt(pos + 2)
					: null;

			if (leftOutside != null) {
				if (leftOutside.nextPos + leftOutside.getMeasuredWidth() >= left.nextPos) {
					leftOutside.nextPos = left.nextPos
							- leftOutside.getMeasuredWidth();
				}
			}

			if (left != null) {
				if (left.currentPos + left.getMeasuredWidth() >= center.currentPos) {
					left.currentPos = center.currentPos
			...
			if (right != null) {
				...
				if (right.currentPos <= center.currentPos
						+ center.getMeasuredWidth()) {
					right.currentPos = center.currentPos
							+ center.getMeasuredWidth();
				}
			}

			if (rightOutside != null) {
				if (rightOutside.prevPos <= right.prevPos
						+ right.getMeasuredWidth()) {
					rightOutside.prevPos = right.prevPos
							+ right.getMeasuredWidth();
				}
			}
		}
	...
	private void applyStyles() {
		...
		for (int i = 0; i < count; i++) {
			...
			tab.setPadding(mTabPaddingLeft, mTabPaddingTop, mTabPaddingRight,
					mLineHeight + mTabPaddingBottom - 4);
	...
	public void refreshTitles() {
		...
		for (int i = 0; i < count; i++) {
			ViewPagerTab tab = (ViewPagerTab) getChildAt(i);
			tab.setText(((ViewPagerTabProvider) mPager.getAdapter())
					.getTitle(i));
		}
