...
import com.qianseit.westore.ui.pulltorefresh.lib.internal.LoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		final int ANIMATION_DURATION_MS = 190;
		final int ANIMATION_FPS = 1000 / 60;
	...
	final float FRICTION = 2.0f;

	final int PULL_TO_REFRESH = 0x0;
	final int RELEASE_TO_REFRESH = 0x1;
	final int REFRESHING = 0x2;
	final int MANUAL_REFRESHING = 0x3;

	static public final int MODE_PULL_DOWN_TO_REFRESH = 0x1;
	static public final int MODE_PULL_UP_TO_REFRESH = 0x2;
	public final int MODE_BOTH = 0x3;
			...
			if (isBeingDragged) {
				...
				if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
					...
					if (currentMode == MODE_PULL_DOWN_TO_REFRESH) {
						...
						onRefreshListener.onRefreshMore();
						smoothScrollTo(0);
					}
				} else {
					smoothScrollTo(0);
	...
	protected void setRefreshingInternal(boolean doScroll) {
		...
		if (doScroll && currentMode == MODE_PULL_DOWN_TO_REFRESH) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_refreshMode)) {
			mode = a.getInteger(R.styleable.PullToRefresh_refreshMode, MODE_BOTH);
		}
