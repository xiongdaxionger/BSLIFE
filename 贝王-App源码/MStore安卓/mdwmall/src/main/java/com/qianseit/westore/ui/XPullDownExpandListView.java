...
public class XPullDownExpandListView extends ExpandableListView implements OnScrollListener {
	...
	// -- 头部的View
	private XPullDownListViewHeader mHeaderView;
	...
	// -- 底部的View
	private XPullDownListViewFooter mFooterView;
	...
	private final static float OFFSET_RADIO = 1.8f; // support iOS like pull
													// feature.
	public XPullDownExpandListView(Context context) {
		super(context);
		initWithContext(context);
	}

	public XPullDownExpandListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initWithContext(context);
	}

	public XPullDownExpandListView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initWithContext(Context context) {
		...
		// 初始化头部View
		mHeaderView = new XPullDownListViewHeader(context);
		mHeaderViewContent = (RelativeLayout) mHeaderView.findViewById(R.id.xlistview_header_content);
		mHeaderTimeView = (TextView) mHeaderView.findViewById(R.id.xlistview_header_time);
		addHeaderView(mHeaderView);// 把头部这个视图添加进去

		// 初始化底部的View
		mFooterView = new XPullDownListViewFooter(context);
	...
	public void setAdapter(ExpandableListAdapter adapter) {
		// 确定XPullDownListViewFooter是最后底部的View, 并且只有一次
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();// 显示
			mFooterView.setState(XPullDownListViewFooter.STATE_NORMAL);
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(XPullDownListViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(XPullDownListViewHeader.STATE_NORMAL);
			}
		}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // 高度足以调用加载更多
				mFooterView.setState(XPullDownListViewFooter.STATE_READY);
			} else {
				mFooterView.setState(XPullDownListViewFooter.STATE_NORMAL);
			}
		}
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(XPullDownListViewFooter.STATE_LOADING);
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
					...
					mHeaderView.setState(XPullDownListViewHeader.STATE_REFRESHING);
					if (mListViewListener != null) {
	...
	public void setXPullDownListViewListener(IXListViewListener l) {
		mListViewListener = l;
	}
