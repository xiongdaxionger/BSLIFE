...
public class Util extends Comm {
	...
	// //////////////////////////////加载配置文件中的信息////////////////////////////////
	public static String loadOptionString(Context context, String name, String defValue) {
	...
	public static boolean loadOptionBoolean(Context context, String name, boolean defValue) {
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
		return prefs.getBoolean(name, defValue);
	}

	public static long loadOptionLong(Context context, String name, long defValue) {
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);

		return prefs.getLong(name, defValue);
	}

	public static float loadOptionFloat(Context context, String name, float defValue) {
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
	 ...
	 * @return false 失败
	 */
	public static final boolean copyAsset2File(Context context, String assetName, String to) {
		try {
	...
	public static boolean isPhoneNumber(String number) {
		...
		Pattern PHONE = Pattern.compile("(\\+[0-9]+[\\- \\.]*)?" + "(\\([0-9]+\\)[\\- \\.]*)?" + "([0-9][0-9\\- \\.][0-9\\- \\.]+[0-9])");
	...
	public static String getFormatedCurrenTime() {
		String time = android.text.format.DateFormat.format("yyyyMMddkkmmss", System.currentTimeMillis()).toString();
		return time;
	}

	// 格式化后的时间
	public static String getFormatedCurrenTime(long longTime) {
		String time = android.text.format.DateFormat.format("yyyy年MM月dd日", System.currentTimeMillis()).toString();
		return time;
	}
	...
	public static long getTimeFromString(String timeStr) {
		try {
			return (new SimpleDateFormat("yyyyMMddkkmmss")).parse(timeStr).getTime();
	 ...
	 * @param siteUrl
	 */
	public static void shareText(Context context, String subject, String msg, String path) {
		...
		Intent sendIntent = Intent.createChooser(intent, context.getString(R.string.share));
	...
	public static void openAppInPlayStore(Context a, String pkgname) {
		...
		try {
			...
			try {
				// 打开浏览器
				Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(PLAY_STORE + pkgname));
				intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				a.startActivity(intent);
			} catch (Exception ex) {
			}
	 ...
	 * @param notification
	 */
	public static void showNotify(Context context, int notifyId, Notification notification) {
	...
	public static String readAssetStr(Context context, String fileName) {
		String result = "";
		try {
			BufferedReader bis = new BufferedReader(new InputStreamReader(context.getAssets().open(fileName)));
			String str = null;
	...
	public static void openSoftInputMethod(Context mContext, View view) {
		try {
			((InputMethodManager) mContext.getSystemService(Context.INPUT_METHOD_SERVICE)).showSoftInput(view, 0);// 打开软键盘
		} catch (Exception e) {
		}
	}
	 ...
	 *            当前焦点
	 */
	public static void showSoftInputMethod(Activity mContext, View view) {
		try {
			// 打开软键盘
			((InputMethodManager) mContext.getSystemService(Context.INPUT_METHOD_SERVICE)).showSoftInputFromInputMethod(view.getWindowToken(), InputMethodManager.SHOW_FORCED);
		} catch (Exception e) {
		}
	}
	...
	public static void uninstallApp(Context context, String pkgName) {
		...
		Intent uninstallIntent = new Intent(Intent.ACTION_DELETE, Uri.fromParts("package", pkgName, null));
	 ...
	 * @return
	 */
	public static Drawable getDrawableList(Context ctx, int resid, float bmTransate) {
		return getDrawableList(BitmapFactory.decodeResource(ctx.getResources(), resid), bmTransate);
	}
	...
	public static Drawable getDrawableList(Drawable drawable, float bmTransate) {
		return getDrawableList(((BitmapDrawable) drawable).getBitmap(), bmTransate);
	}
	...
	public static Drawable getDrawableList(Bitmap bm, float bmTransate) {
		...
		mDrawable.addState(new int[] { android.R.attr.state_pressed }, new BitmapDrawable(Resources.getSystem(), pressed_bm));
		mDrawable.addState(new int[] { android.R.attr.state_focused }, new BitmapDrawable(Resources.getSystem(), pressed_bm));
		mDrawable.addState(StateSet.WILD_CARD, new BitmapDrawable(Resources.getSystem(), bm));
		return mDrawable;
	}
	...
	public static Drawable getDrawableList(Drawable src) {
		...
		pressDrawable.setColorFilter(Color.parseColor("#66000000"), PorterDuff.Mode.SRC_ATOP);
		StateListDrawable mDrawable = new StateListDrawable();
		mDrawable.addState(new int[] { android.R.attr.state_pressed }, pressDrawable);
		mDrawable.addState(new int[] { android.R.attr.state_focused }, pressDrawable);
	...
	public static Drawable getDrawableListReverse(Bitmap bm, float bmTransate) {
		...
		StateListDrawable mDrawable = new StateListDrawable();
		mDrawable.addState(new int[] { android.R.attr.state_pressed }, new BitmapDrawable(Resources.getSystem(), bm));
		mDrawable.addState(new int[] { android.R.attr.state_focused }, new BitmapDrawable(Resources.getSystem(), bm));
		mDrawable.addState(StateSet.WILD_CARD, new BitmapDrawable(Resources.getSystem(), pressed_bm));
	...
	private static Bitmap getTranslucentBitmap(Bitmap map, float translate) {
		Bitmap pressed_map = Bitmap.createBitmap(map.getWidth(), map.getHeight(), Config.ARGB_8888);
		Canvas pressed_canvas = new Canvas(pressed_map);
		Paint p = new Paint();
		p.setColorFilter(new ColorMatrixColorFilter(getContrastTranslateOnly(translate)));
	 ...
	 * @return
	 */
	public static Bitmap createCornerRadiusBitmap(Bitmap bitmap, int cornerRadius) {
		try {
	...
	public static void saveBitmap(String fileName, Bitmap bitmap) {
		saveBitmap(null, fileName, bitmap, "");

	}

	public static void saveBitmap(Context c, String fileName, Bitmap bitmap, String path) {
		File file = null;
		if (bitmap == null) {
			return;
		}
		...
		try {
			...
			if (c != null) {
				...
				c.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, data));
			}
		}
	}

	public static String getPath() {
		String path = Comm.doFolder + "ImageCache";
		return path;
	}
	 ...
	 * @param receiver
	 */
	public static void unregistReceiverSafety(Context context, BroadcastReceiver receiver) {
		try {
	...
	public static String calculateRemainTime(long remainTime) {
		...
		String time = "";
		if (day > 0) {
			time = day + "天" + hour_decade + hour_unit + ":" + min_decade + min_unit + ":" + sec_decade + sec_unit;
		} else {
			if (hour_decade == 0 && hour_unit == 0) {
				time = "" + min_decade + min_unit + ":" + sec_decade + sec_unit;
			} else {
				time = "" + hour_decade + hour_unit + ":" + min_decade + min_unit + ":" + sec_decade + sec_unit;
			}
		}
		return time;
	}
	...
	/**
	 * author chanson creatTime 2015-10-20 下午3:49:42
	 * 
	 * @param remainTime
	 * @return 如1天、12小时或20分钟或12秒等 return String
	 */
	public static String calculateRemainTimeSimple(long remainTime) {
		long seconds = remainTime % 60;
		long minutes = remainTime / 60 % 60;
		long hours = remainTime / (60 * 60) % 24;
		long day = remainTime / (60 * 60 * 24);
	 ...
	 * @return 如1天、12小时或20分钟或12秒等 return String
	 */
	public static String calculateMinutesSimple(long remainTime) {
		long seconds = remainTime % 60;
		long minutes = remainTime / 60 % 60;
		long hours = remainTime / (60 * 60) % 24;
		long day = remainTime / (60 * 60 * 24);
	 ...
	 * 
	 * @param remainTime
	 * @return 如1天、12小时20分钟等 return String
	 */
	public static String calculateMinutes(long remainTime) {
		long seconds = remainTime % 60;
		long minutes = remainTime / 60 % 60;
		long hours = remainTime / (60 * 60) % 24;
		long day = remainTime / (60 * 60 * 24);
	...
	public static Bitmap CreateOneDCode(Context context, String content) {
		...
		try {
			matrix = new MultiFormatWriter().encode(content, BarcodeFormat.CODE_128, dip2px(context, 250), dip2px(context, 50));
	 ...
	 *             WriterException异常
	 */
	public static Bitmap CreateTwoDCodeDip(Context context, String content, int dipWidth, int dipHeight) {
		return CreateTwoDCode(context, content, dip2px(context, dipWidth), dip2px(context, dipWidth));
	}
	 ...
	 *             WriterException异常
	 */
	public static Bitmap CreateTwoDCode(Context context, String content, int width, int height) {
		try {
			if (width < 100) {
				width = 200;
			}
			...
			height = matrix.getHeight(); // 二维矩阵转为一维像素数组,也就是一直横着排了
			int[] pixels = new int[width * height];
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					if (matrix.get(x, y)) {
			...
			Bitmap bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888); // 通过像素数组生成bitmap,具体参考api
			bitmap.setPixels(pixels, 0, width, 0, 0, width, height);
			return bitmap;
		} catch (WriterException e) {
			e.printStackTrace();
	 ...
	 *            多图片的本地路径
	 */
	public static void shareImagesToWx(Context context, ArrayList<String> filesUrl) {
		File file = null;
		File desFile = null;
		ArrayList<File> files = new ArrayList<File>();
		for (int i = 0; i < filesUrl.size(); i++) {
			file = new File(filesUrl.get(i));
			desFile = new File(Run.doFolder, "share_image" + i + ".jpg");
			try {
		...
		shareToTimeLineUI(context, files);
	}
	 ...
	 *            多图片
	 */
	public static void shareToTimeLineUI(Context context, ArrayList<File> files) {
	 ...
	 * @return
	 */
	public static String roundNum(Double v, int scale) {
		if (scale < 0) {
	...
	public static List<String> getBackTime(int dayNum) {
		...
		for (int i = 0; i <dayNum; i++) {
			...
			timeArray.add(sim.format(rightNow.getTime()));
		}
		return timeArray;
	}
}