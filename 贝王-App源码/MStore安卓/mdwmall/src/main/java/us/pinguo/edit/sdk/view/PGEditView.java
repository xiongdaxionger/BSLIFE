...
public class PGEditView implements IPGEditView {
	...
	private final long ANIMATION_TIME = 250l;
	...
	public void initView(Activity activity) {
		...
		mCenterLayoutParent.setVisibility(View.VISIBLE);
	...
	public void moveTopAndCenterToUpWithAnimation() {
		if (Build.VERSION.SDK_INT >= 11) {
			float centerMoveHeight = mActivity.getResources().getDimension(R.dimen.pg_sdk_edit_center_move_top_height);
			...
			// float topHeight =
			// mActivity.getResources().getDimension(R.dimen.pg_sdk_edit_top_height);
			// ObjectAnimator topAnimator =
			// ObjectAnimator.ofFloat(mFirstTopView, "y", 0f, -topHeight);
			...
			mCenterLayoutParent.setPadding(mCenterLayoutParent.getPaddingLeft(), mCenterLayoutParent.getPaddingTop() - centerMoveHeight, mCenterLayoutParent.getPaddingRight(), centerMoveHeight
					+ mCenterLayoutParent.getPaddingBottom());
			if (null != mMenuListener) {
				mMenuListener.onShowSecondAnimationEnd();
			}
		}
	...
	@Override
	public void addSecondEffectChildViews(List list, float showCount, View.OnClickListener onThirdClickListener, View.OnClickListener onScrollClickListener, String maskIcon, View lastSelectedView) {

		PGEditEffectHoriScrollItemAdapter baseHoriScrollItemAdapter = new PGEditEffectHoriScrollItemAdapter(mSecondHorizontalLayout, -1);
	...
	public void showFirstImageViewPhoto(List mPGEditMenusBeanList, float showCount, View.OnClickListener onClickListener) {
		...
		baseHoriScrollItemAdapter.setOnItemViewClickListener(onClickListener);
		if (mPGEditMenusBeanList.size() > 1) {
			mPGEditMenusBeanList.remove(1);
		}
		if (mPGEditMenusBeanList.size() > 2) {
			mPGEditMenusBeanList.remove(2);
		}
		...
		// mActivity.findViewById(R.id.first_top).setVisibility(View.VISIBLE);
		//
		// PGEditBaseHoriScrollItemAdapter baseHoriScrollItemAdapter = new
	...
	@Override
	public void addThirdEffectChildViews(List list, float showCount, View.OnClickListener onThirdClickListener, View.OnClickListener onScrollClickListener, String maskIcon, View view,
			View lastSelectedView) {

		PGEditEffectHoriScrollItemAdapter baseHoriScrollItemAdapter = new PGEditEffectHoriScrollItemAdapter(mThirdHorizontalLayout, -1);
		baseHoriScrollItemAdapter.setOnItemViewClickListener(onThirdClickListener);
		baseHoriScrollItemAdapter.setOnScrollClickListener(onScrollClickListener);
		baseHoriScrollItemAdapter.setMaskResource(maskIcon);
		baseHoriScrollItemAdapter.setData(list);
	...
	private class SaveTipsDialog extends Dialog {
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			LinearLayout.LayoutParams descViewParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
			...
			LinearLayout.LayoutParams vDividerParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 1);
			...
			RelativeLayout.LayoutParams hDividerParams = new RelativeLayout.LayoutParams(1, Math.round(48 * density));
			...
			RelativeLayout.LayoutParams confirmButtonParams = new RelativeLayout.LayoutParams((width - Math.round(80 * density)) / 2, ViewGroup.LayoutParams.WRAP_CONTENT);
			...
			RelativeLayout.LayoutParams closeButtonParams = new RelativeLayout.LayoutParams((width - Math.round(80 * density)) / 2, ViewGroup.LayoutParams.WRAP_CONTENT);
